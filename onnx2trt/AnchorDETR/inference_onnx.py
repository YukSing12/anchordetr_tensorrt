...
from engine import evaluate, train_one_epoch, evaluate_onnx
...
def get_args_parser():
    ...
    parser.add_argument('--num_images', default=5000, type=int, help="numbers of image to be inferenced")
     ...
     # ONNX params
    parser.add_argument('--onnx_path', default=['AnchorDETR.onnx'], type=str, nargs='+')
...
def main(args):
    ...
    if args.eval:
        if args.device == 'cpu':
            for onnx_file in args.onnx_path:
                ort_session.append(onnxruntime.InferenceSession(onnx_file, providers=["CPUExecutionProvider"])) 
            print("CPU as backend")
        else:
            for onnx_file in args.onnx_path:
                ort_session.append(onnxruntime.InferenceSession(onnx_file, providers=["CUDAExecutionProvider"]))
            print("CUDA as backend")
        test_stats, coco_evaluator = evaluate_onnx(ort_session, criterion, postprocessors,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('AnchorDETR ONNX inference script', parents=[get_args_parser()])
