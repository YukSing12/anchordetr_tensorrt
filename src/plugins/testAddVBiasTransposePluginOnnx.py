...
soFilePath      = './AddVBiasTransposePlugin.so'
nBS             = 1
nHead           = 8
nH              = 20
nW              = 32
...
def test_cpu(value):
    _v_0 = value + globalQBias
    _v_1 = np.transpose(_v_0, [1,2,0,3])
    _v_2 = _v_1.reshape([_v_1.shape[0], _v_1.shape[1], nHead, nSL // nHead])
    _v_3 = np.transpose(_v_2, [2,0,1,3])
    _v_4 = _v_3.reshape([_v_3.shape[0], _v_3.shape[1], _v_3.shape[2] * nSL // nHead])
    return _v_4

def get_onnx():
    onnx_file = "temp.onnx"
    shape = ('B', 'H', 'W' ,nSL)
    value = gs.Variable(name="value", dtype=npDataType, shape=shape)
    ...
    plugin = gs.Node(op="AddVBiasTransposePlugin", 
                        name="AddVBiasTransposePlugin_1", 
                        inputs=[value, q_bias], 
                        outputs=[y])
    graph = gs.Graph(nodes=[plugin], inputs=[value], outputs=[y])
...
def run():
    ...
    profile.set_shape(inputTensor.name, (nBS, nH, nW, nSL), (nBS, nH, nW, nSL), (nBS*2, nH*2, nW*2, nSL))  
